...
Let $G$ be a group and $H$ be a subgroup of $G$. For all $x, y ∈ G$, the relation $x∼y ∈ H$, if and only if $x^{−1}y ∈ H$, respectively XXXX x ∼ yifandonlyifyx−1 ∈H,isanequivalence relation. An equivalence class for this relation is denoted by xH = {xh | h ∈ H}, respectively Hx = {hx | h ∈ H} and are called respectively left and right cosets of H. The numbers of classes for both relations are the same. This invariant is called the index of H in G and is denoted by [G : H].
