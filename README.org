...
Given two groups $(G,*)}(G,∗)$ and {\displaystyle (H,\odot ),}(H,⊙), a group isomorphism from {\displaystyle (G,*)}(G,∗) to {\displaystyle (H,\odot )}(H,⊙) is a bijective group homomorphism from {\displaystyle G}G to {\displaystyle H.}H. Spelled out, this means that a group isomorphism is a bijective function {\displaystyle f:G\to H}f:G→H such that for all {\displaystyle u}u and {\displaystyle v}v in {\displaystyle G}G it holds that
